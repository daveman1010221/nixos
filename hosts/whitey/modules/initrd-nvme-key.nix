...
let
  ...
  nvmeKeyScript = pkgs.runCommand "nvme-hw-key" { } ''
    mkdir -p $out
    cat > $out/nvme-hw-key <<'EOF'
#!/bin/sh
set -euo pipefail

echo "[nvme-hw-key] Waiting for secrets_crypt"
udevadm settle --timeout=10
for i in $(seq 1 10); do
  if [ -e /dev/mapper/secrets_crypt ]; then break; fi
  sleep 1
done
...
in {
  ...
  boot.initrd.systemd.units."nvme-hw-key.service" = {
    ...
    text = ''
      ...
      After=systemd-cryptsetup@secrets_crypt.service systemd-udev-settle.service
      Before=sysroot.mount

      [Service]
      Type=oneshot
      ExecStart=${nvmeKeyScript}/nvme-hw-key
    '';
  };
}
